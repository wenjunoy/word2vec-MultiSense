...
char syn1_file[MAX_STRING], syn1neg_file[MAX_STRING], syn0_file[MAX_STRING];
struct vocab_word *vocab;
int binary = 0, cbow = 0, debug_mode = 2, window = 5, min_count = 5, num_threads = 1, min_reduce = 1;
int *vocab_hash;
long long vocab_max_size = 1000, vocab_size = 0, layer1_size = 100;
long long train_words = 0, word_count_actual = 0, file_size = 0, classes = 0, iter = 1;
...
int AddWordToVocab(char *word) {
  ...
  vocab[vocab_size].df = 0;
  vocab[vocab_size].tag = 0;
  vocab_size++;
...
void ReadVocab() {
  ...
  printf("read vocab from vocab file.\n");
  for (a = 0; a < vocab_hash_size; a++) vocab_hash[a] = -1;
  vocab_size = 0;
  while (1) {
    ReadWord(word, fin);
...
void TrainModel() {
  long a, b, c, d;
  char output_nobin_file[MAX_STRING];
  FILE *fo,*fo_nobin;
  ...
  strcpy(output_nobin_file, output_file);
  fo_nobin = fopen(strcat(output_nobin_file, ".nobin"), "wb");
  if (fo == NULL || fo_nobin == NULL) {
  ...
  if (classes == 0) {
    ...
    fprintf(fo, "%lld %lld\n", vocab_size, layer1_size);
    fprintf(fo_nobin, "%lld %lld\n", vocab_size, layer1_size);
    for (a = 0; a < vocab_size; a++) {
      if (vocab[a].word != NULL) {
        fprintf(fo, "%s ", vocab[a].word);
        fprintf(fo_nobin, "%s ", vocab[a].word);
      }
      // if (binary)
        for (b = 0; b < layer1_size; b++) fwrite(&syn0[a * layer1_size + b], sizeof(real), 1, fo);
        for (b = 0; b < layer1_size; b++) fprintf(fo_nobin, "%lf ", syn0[a * layer1_size + b]);
...
int main(int argc, char **argv) {
  ...
  if (argc == 1) {
    ...
    printf("\nExamples:\n");
    printf("./word2vec -train data.txt -output vec.txt -debug 2 -size 200 -window 5 -sample 1e-4 -negative 5 -hs 0 -binary 0 -cbow 1 -iter 5\n\n");
    return 0;
  }
  output_file[0] = 0;
  save_vocab_file[0] = 0;
