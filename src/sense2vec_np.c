...
struct vocab_word {
  ...
  char pos;
  char kwt; // this word has kwt means.
};


char train_file[MAX_STRING], output_file[MAX_STRING];
char word2vec_file[MAX_STRING] = "../data/text8.sense2vec.vec";
...
int *vocab_hash;
long long vocab_max_size = 1000, vocab_size = 0, layer1_size = 100, sensevb_size = 0;
long long train_words = 0, word_count_actual = 0, file_size = 0, classes = 0;
real alpha = 0.025, starting_alpha, sample = 0, lambda_sim=-0.5;
...
void SaveVocab() {
  ...
  for (i = 0; i < vocab_size; i++) fprintf(fo, "%s %lld %d\n", vocab[i].word, vocab[i].cn, vocab[i].kwt);
  fclose(fo);
}
...
void InitNet() {
  ...
  a = posix_memalign((void **)&syn0sense, 128, (long long)vocab_size *MAX_SENSE * layer1_size * sizeof(real));
  if (syn0sense == NULL) {printf("Memory allocation failed\n"); exit(1);}

  a = posix_memalign((void **)&syn0mu, 128, (long long)vocab_size *MAX_SENSE * layer1_size * sizeof(real));
  if (syn0mu == NULL) {printf("Memory allocation failed\n"); exit(1);}

  a = posix_memalign((void **)&sense_num, 128, (long long)vocab_size* MAX_SENSE * sizeof(int));
  ...
  // skip-gram word2vec initial
  if (init_wordvec > 0) InitWord2Vec();

  for (b = 0; b < layer1_size; b++) for (a=0; a< vocab_size;a++) for (k = 0; k < MAX_SENSE; k ++)
    syn0sense[(a *MAX_SENSE + k)*layer1_size +b] = (rand() / (real)RAND_MAX -0.5) / layer1_size;
  for (b = 0; b < layer1_size; b++) for (a=0; a< vocab_size;a++) for (k = 0; k < MAX_SENSE; k ++)
    syn0mu[(a *MAX_SENSE + k)*layer1_size +b] = 0.0;


  for (a=0; a< vocab_size;a++) for(k = 0 ;k <MAX_SENSE;k++)
    sense_num[a*MAX_SENSE+k] = 0;
  printf("Init the Networds.\n");
  CreateBinaryTree();
}
...
void *TrainModelThread(void *id) {
  ...
  long long l1, l1_s, l2, c, target, label;
  long long k, window_words_num, arg_max, sense_i;
  ...
  real *similarity = (real*)calloc(MAX_SENSE, sizeof(real));
  ...
  while (1) {
    ...
    for (c = 0; c < MAX_SENSE; c++) similarity[c] = 0.0;
    next_random = next_random * (unsigned long long)25214903917 + 11;
    ...
    //normalize
    // len_c = 0.0;
    // for (c = 0; c < layer1_size; c++) len_c += context_vec[c] * context_vec[c];
    // for (c = 0; c < layer1_size; c++) context_vec[c] /= sqrt(len_c);
    ...
    for (a = b; a < window * 2 + 1 - b; a++) if (a != window) {
      ...
      for (c = 0; c < MAX_SENSE; c++) similarity[c] = 0;
      arg_max = -1;
      max = -100000.0;
      for (k = 0;k < vocab[last_word].kwt;k ++) {
        sense_i = last_word * MAX_SENSE + k;
        for (c = 0;c < layer1_size; c++) {
          similarity[k] += context_vec[c] * syn0mu[sense_i* layer1_size + c];
        }
        //printf("%f ", similarity[k]);
        if (similarity[k] > max){
      ...
      if (max < lambda_sim && vocab[last_word].kwt < MAX_SENSE) {
        arg_max = vocab[last_word].kwt;
        vocab[last_word].kwt += 1;
      }
      //printf("arg_max: %d\n",arg_max);

      long long l1_max = last_word * MAX_SENSE + arg_max;
...
void TrainModel() {
  ...
  if (fo == NULL || fo_sense == NULL || f_num == NULL) {
  ...
  if (classes == 0) {
    ...
    long long sense_kwt =0;
    for (a = 0; a <vocab_size; a++) {
      sense_kwt += vocab[a].kwt;
    }

    fprintf(fo_sense, "%lld %lld\n", sense_kwt, layer1_size);
    for (a = 0; a < vocab_size; a++) {
      for (k = 0;k < vocab[a].kwt; k++){
        sprintf(str, "%ld", k);
        if (vocab[a].word != NULL) {
         fprintf(fo_sense, "%s_%s ", vocab[a].word, str);
        }
        if (binary) for (b = 0; b < layer1_size; b++) fwrite(&syn0sense[(a*MAX_SENSE+k) * layer1_size + b], sizeof(real), 1, fo_sense);
        else for (b = 0; b < layer1_size; b++) fprintf(fo_sense, "%lf ", syn0sense[(a*MAX_SENSE+k) * layer1_size + b]);
        fprintf(fo_sense, "\n");
      }
    }
...
int main(int argc, char **argv) {
  ...
  if ((i = ArgPos((char *)"-lambda-sim", argc, argv)) > 0) lambda_sim = atof(argv[i + 1]);
  if ((i = ArgPos((char *)"-init-sense0", argc, argv)) > 0) init_sense = atoi(argv[i + 1]);
  if ((i = ArgPos((char *)"-wordvec", argc, argv)) > 0) {strcpy(word2vec_file, argv[i + 1]); init_wordvec = 1;}
